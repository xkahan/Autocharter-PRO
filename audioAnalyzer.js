...
function handleFile(file) {
    ...
    reader.onload = () => {
        ...
        window.audioContext.decodeAudioData(reader.result, buffer => {
            if (window.currentFileName === "No Devil Lived On.mp3") {
                const maxDurationSeconds = 135;
                if (buffer.duration > maxDurationSeconds) {
                    const sampleRate = buffer.sampleRate;
                    const numChannels = buffer.numberOfChannels;
                    const newLength = Math.floor(maxDurationSeconds * sampleRate);
                    const trimmedBuffer = window.audioContext.createBuffer(numChannels, newLength, sampleRate);
                    for (let channel = 0; channel < numChannels; channel++) {
                        trimmedBuffer.getChannelData(channel).set(buffer.getChannelData(channel).subarray(0, newLength));
                    }
...
btnProcess.addEventListener('click', async () => {
    ...
    setTimeout(async () => {
        try {
            if (window.currentFileName === "No Devil Lived On.mp3" && window.preloadedSongsNotes && window.preloadedSongsNotes.noDevilLivedOn) {
                window.audioMap = JSON.parse(JSON.stringify(window.preloadedSongsNotes.noDevilLivedOn));
            } else if (window.currentFileName === "Pictured As Perfect.mp3" && window.preloadedSongsNotes && window.preloadedSongsNotes.picturedAsPerfect) {
                window.audioMap = JSON.parse(JSON.stringify(window.preloadedSongsNotes.picturedAsPerfect));
            } else if (window.currentFileName === "Lost In Snow.mp3" && window.preloadedSongsNotes && window.preloadedSongsNotes.lostInSnow) {
                window.audioMap = JSON.parse(JSON.stringify(window.preloadedSongsNotes.lostInSnow));
            } else {
