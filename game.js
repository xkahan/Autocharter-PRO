...
function resizeCanvas() {
    ...
    // Redraw frame if paused or ready
...
window.getGameState = function () {
    return { score, combo, health, colWidth, hitZoneY, colorMap, splashStyle, noteSize, noteShape };
};
...
function updateHealth(amount) {
    ...
    if (health <= 0) {
        if (window.localMode && window.localMode.active) {
            ...
            heldKeys.clear();
            showFeedback('(MUERTE)', 'feedback-miss');
...
function gameOver() {
    ...
    if (window.onlineMode && window.onlineMode.active) {
        ...
        // Draw (Muerte) under local player's track
        ctx.save();
        ...
        ctx.fillText('(MUERTE)', localX, canvas.height - 30);
        ctx.restore();
...
window.handleOpponentDeath = function () {
    ...
    if (window.onlineMode && window.onlineMode.active) {
        ...
        ctx.fillText('(MUERTE)', oppX, canvas.height - 30);
        ctx.restore();
    }
...
function drawGame(timestamp, forceRedraw = false) {
    ...
    // Update layout coordinates from DOM to guarantee alignment
    const p1Window = document.getElementById('game-window-p1');
    const p2Window = document.getElementById('game-window-p2');
    if (p1Window) {
        const p1Rect = p1Window.getBoundingClientRect();
    ...
    // Title Typewriter Effect for Extreme Maps
...
window.drawReadyState = function () {
    ...
    if (isTwoPlayer) {
        ...
        if (isLocalCoop && window.localMode.p1Died) {
            ...
            ctx.fillText('(MUERTE)', p1CenterX, canvas.height - 40);
            ctx.restore();
        }
    }
    ...
    if (isTwoPlayer) {
        ...
        if (isLocalCoop && window.localMode.p2Died) {
            ...
            ctx.fillText('(MUERTE)', p2CenterX, canvas.height - 40);
            ctx.restore();
        }
...
function showResults() {
    ...
    if (totalNotes > 0 && countPerfect === totalNotes) {
    ...
    if ((window.onlineMode && window.onlineMode.active) || (window.localMode && window.localMode.active)) {
        ...
        let oppRank = 'D';
        let oppRankClass = 'rank-D';
        if (oppAccuracy >= 98) { oppRank = 'SS'; oppRankClass = 'rank-SS'; }
        else if (oppAccuracy >= 90) { oppRank = 'S'; oppRankClass = 'rank-S'; }
        else if (oppAccuracy >= 80) { oppRank = 'A'; oppRankClass = 'rank-A'; }
        else if (oppAccuracy >= 70) { oppRank = 'B'; oppRankClass = 'rank-B'; }
        else if (oppAccuracy >= 50) { oppRank = 'C'; oppRankClass = 'rank-C'; }
...
function initForest() {
    ...
    // Populate fixed snow particles pool (400 total)
    const countPerLayer = [100, 120, 100, 60, 20];
    for (let l = 0; l < 5; l++) {
...
function changeActualScreen(screenName) {
    ...
    if (screenName === 'main-menu') {
        ...
        const isMobile = ('ontouchstart' in window) || navigator.maxTouchPoints > 0 || /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.innerWidth <= 768;
        if (touchContainer && isMobile) {
...
function updatePlayer2Health(val) {
    ...
    if (window.localMode.p2Health <= 0 && window.localMode && window.localMode.active) {
        ...
        window.localMode.p2HeldKeys.clear();
        showP2Feedback('(MUERTE)', 'feedback-miss');
...
function initTouchControls() {
    ...
    // Detección dinámica de tipo de entrada: Ocultar paneles al usar teclado / ratón y mostrarlos al tocar la pantalla
    window.addEventListener('keydown', () => {
        if (!touchContainer.classList.contains('hidden')) {
