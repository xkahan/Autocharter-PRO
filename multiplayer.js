...
(function () {
    ...
    async function registerRoomInLobby(code, hostName) {
        try {
            const now = Date.now();
            let rooms = getLocalPublicRooms().filter(r => now - r.created < 10 * 60 * 1000 && r.code !== code);
            rooms.push({ code: code, host: hostName, created: now });
            saveLocalPublicRooms(rooms);

            const response = await fetch(`https://keyvalue.immanuel.co/api/KeyVal/GetValue/${LOBBY_APP_KEY}/${LOBBY_ROOMS_KEY}`);
            let apiRooms = [];
            if (response.ok) {
                let text = await response.text();
                if (text && text !== '"null"' && text !== 'null' && !text.startsWith('Error')) {
                    if (text.startsWith('"') && text.endsWith('"')) text = JSON.parse(text);
                    try { apiRooms = JSON.parse(text); } catch(e) { apiRooms = []; }
                }
            }
            apiRooms = apiRooms.filter(r => now - r.created < 10 * 60 * 1000 && r.code !== code);
            apiRooms.push({ code: code, host: hostName, created: now });

            const encoded = encodeURIComponent(JSON.stringify(apiRooms));
            await fetch(`https://keyvalue.immanuel.co/api/KeyVal/UpdateValue/${LOBBY_APP_KEY}/${LOBBY_ROOMS_KEY}/${encoded}`, { method: 'POST' });
    ...
    async function removeRoomFromLobby(code) {
        ...
        try {
            const now = Date.now();
            let rooms = getLocalPublicRooms().filter(r => r.code !== code && (now - r.created < 10 * 60 * 1000));
            saveLocalPublicRooms(rooms);
            ...
            apiRooms = apiRooms.filter(r => r.code !== code && (now - r.created < 10 * 60 * 1000));
            const encoded = encodeURIComponent(JSON.stringify(apiRooms));
            await fetch(`https://keyvalue.immanuel.co/api/KeyVal/UpdateValue/${LOBBY_APP_KEY}/${LOBBY_ROOMS_KEY}/${encoded}`, { method: 'POST' });
    ...
    window.refreshRoomsLobby = async function() {
        ...
        let rooms = [];
        const now = Date.now();
        const localRooms = getLocalPublicRooms().filter(r => now - r.created < 10 * 60 * 1000);
        rooms.push(...localRooms);
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (text && text !== '"null"' && text !== 'null' && !text.startsWith('Error')) {
                    if (text.startsWith('"') && text.endsWith('"')) text = JSON.parse(text);
                    try {
                        const apiRooms = JSON.parse(text);
                        if (Array.isArray(apiRooms)) {
                            apiRooms.forEach(ar => {
                                if (now - ar.created < 10 * 60 * 1000 && !rooms.some(r => r.code === ar.code)) {
                                    rooms.push(ar);
                                }
                            });
                        }
        ...
        rooms = rooms.filter(r => now - r.created < 10 * 60 * 1000);
        ...
        conn.on('data', (data) => {
            ...
            switch (data.type) {
                ...
                case 'game-over':
                    ...
                    window.onlineMode.opponent.maxCombo = data.maxCombo;
                    break;
    ...
    window.addEventListener('neonbeat-game-over', () => {
        if (window.onlineMode && window.onlineMode.active && window.onlineMode.conn) {
            ...
            window.onlineMode.conn.send({
                type: 'game-over',
                score: gs.score || 0,
                maxCombo: 0 // Will be set by showResults
            });
        }
    });
